...
public:
    ...
    bool update( const Mat image, CV_OUT Rect2d& boundingBox) CV_OVERRIDE;

    void setInitialMask(const Mat mask) CV_OVERRIDE;
...
bool TrackerCSRTImpl::init(const Mat image, const cv::Rect2d &boundingBox) {
    return initImpl(image, boundingBox);
}

bool TrackerCSRTImpl::update(const cv::Mat image, cv::Rect2d &boundingBox) {
