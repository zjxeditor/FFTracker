...
private:
	...
	// Calculate response based on all the filters and channels weights. Feats are extracted at previous location and scale.
	void CalculateResponse(const std::vector<MatF> &feats, const std::vector<MatCF> &filter, const std::vector<float> &weight, MatF &response) const;
	...
	int pcaCount;						// Feature PCA count.
	int iterations;						// Filter solver iteration count.
	std::vector<float> weightsLearnRates;				// Channel weights learning weight.
	std::vector<float> filterLearnRates;				// Filter model learning weight.

	MatF project;						// Feature PCA project matrix.
	std::vector<std::vector<MatCF>> filters;			// Calculated CSRDCF filters.
	std::vector<std::vector<float>> filterWeights;	// Filter channel weights.
	MemoryArena *arenas;				// Memory arena management

	// Background thread backup.
	std::vector<std::vector<MatCF>> bk_filters;
	std::vector<std::vector<float>> bk_filterWeights;
	bool backgroundUpdateFlag;
