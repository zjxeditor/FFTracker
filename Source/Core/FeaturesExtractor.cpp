...
void FeaturesExtractor::ComputeNormalFromPointCloud(const Vector3f *pointCloud, Vector3f *normal, int width, int height) {
	// Calculate normals
	ParallelFor([&](int64_t y) {
		++y;
...
void FeaturesExtractor::ComputePolarNormalFromPointCloud(const Vector3f *pointCloud, Vector2f *polarNormal, int width, int height) {
	// Calculate normals
	ParallelFor([&](int64_t y) {
		++y;
		const Vector3f *src = pointCloud + y * width + 1;
...
void FeaturesExtractor::ConvertNormalToPolarNormal(const Vector3f* normal, Vector2f* polarNormal, int width, int height) {
	ParallelFor([&](int64_t y) {
...
void FeaturesExtractor::ConvertPolarNormalToNormal(const Vector2f* polarNormal, Vector3f* normal, int width, int height) {
	ParallelFor([&](int64_t y) {
