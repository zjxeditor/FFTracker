//
// Provide accurate string format output support.
//
...
#ifndef CSRT_STRINGPRINT_H
#define CSRT_STRINGPRINT_H
...
inline void stringPrintfRecursive(std::string *s, const char *fmt) {
	...
	while (*c) {
		if (*c == '%') {
			if(c[1] != '%') {
				Critical("Invalid input for \"stringPrintfRecursive\" method.");
			}
			++c;
		}
		*s += *c++;
